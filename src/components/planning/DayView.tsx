...
import { haptic, DAY_SHORT, cn } from '@/lib/utils'
...
export default function DayView({ dayIdx }: Props) {
  ...
  const openMealDetail = (dIdx: number, sk: SlotKey, meal: import('@/types').Meal) => {
        ...
        return (
          <div key={period} className="bg-card rounded-2xl border-[1.5px] border-border p-3 flex flex-col gap-1">
            ...
            <OptionalSlot
              ...
              onPress={() =>
                entreeMeal
                  ? openMealDetail(dayIdx, entreeKey, entreeMeal)
            ...
            {mainMeal ? (
              <MealCard
                meal={mainMeal}
                period={period}
                onPress={() => openMealDetail(dayIdx, slotKey, mainMeal)}
              />
            ) : (
              <MealAddSlot
                period={period}
            ...
            <OptionalSlot
              ...
              onPress={() =>
                dessertMeal
                  ? openMealDetail(dayIdx, dessertKey, dessertMeal)
