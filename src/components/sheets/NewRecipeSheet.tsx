...
function guessEmoji(category: string, name: string): string {
  ...
  if (/chicken/.test(c)) return '🍗'
  if (/beef|lamb|steak/.test(c)) return '🥩'
  if (/seafood|fish|salmon|shrimp|tuna/.test(c)) return '🐟'
  if (/pasta|spaghetti|lasagne|penne/.test(c)) return '🍝'
  if (/dessert|cake|cookie|sweet|pudding/.test(c)) return '🍰'
  if (/breakfast|pancake|waffle/.test(c)) return '🥞'
  if (/vegetarian|vegan|salad/.test(c)) return '🥗'
  if (/pizza/.test(c)) return '🍕'
  if (/soup|stew/.test(c)) return '🍲'
  if (/burger/.test(c)) return '🍔'
  if (/curry/.test(c)) return '🍛'
  if (/taco|mexican/.test(c)) return '🌮'
  return '🍽️'
}
...
export default function NewRecipeSheet() {
  ...
  const handleSelectMeal = (meal: MealDBMeal) => {
    ...
    setEmoji(guessEmoji(meal.strCategory, meal.strMeal))
  ...
  const handleSave = () => {
    ...
    // Reset
    setName(''); setEmoji('🍽️'); setTime(''); setTimeCustom(false)
        ...
        <div>
          ...
          <div className="flex gap-2 mb-4">
            ...
            <button
              ...
              className="px-4 py-3 bg-terra text-white rounded-2xl text-sm font-extrabold active:scale-95 transition-transform disabled:opacity-50"
            >
              {importLoading ? '…' : '🔍'}
            </button>
          </div>
          ...
          <div className="space-y-2.5 mb-4">
            ...
            <div className="flex gap-2">
              <input
                ...
                className="w-14 px-3 py-3 bg-card border-[1.5px] border-border rounded-2xl text-xl text-center outline-none focus:border-terra transition-colors"
              />
              <input
                type="text"
